...
You must set up a read-only account in Cisco UCS Manager, Cisco Nexus switches, and NetApp ONTAP. The agent uses these accounts to collect configuration data from each device.
...
You must deploy the Converged Systems Advisor agent on a VMware ESXi server. The agent collects configuration data about each device in your FlexPod converged infrastructure and sends to the Converged Systems Advisor portal.
...
.. Enter the user name and password of the read-only account that you previously created.
